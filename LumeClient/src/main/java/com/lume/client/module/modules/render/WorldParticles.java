...
public class WorldParticles extends Module {
    ...
    public void onTick() {
        ...
        // per-type look modifiers (each type looks clearly different)
        float gMul = 1f, sMul = 1f, tMul = 1f, lMul = 1f;
        switch (type.index) {
            case 1 -> { sMul = 0.45f; tMul = 2.5f; lMul = 0.5f; }                // Spark — tiny, fast, jittery
            case 2 -> { gMul = -1.5f; sMul = 0.9f; tMul = 1.6f; rgb = 0xFF7A2E; } // Ember — rising, orange
            case 3 -> { gMul = 0.5f;  sMul = 1.3f; tMul = 0.3f; rgb = 0xFFFFFF; } // Snow — falling white flakes
